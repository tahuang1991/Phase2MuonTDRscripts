...
def getEffAndRate(pt, fractions, etamin, etamax, wfilename, filedir, condoroutroot, SBPlots, useGE21, doPlots=True, doLUTs=False, doRates=True):#uncertainty?
	...
	for line in fileopen:
	    ...
	    n_signal_tot = float(allnums[4]); nsignal_h = n_signal_tot; nsignal_d = n_signal_tot; nsignal_p = n_signal_tot
	    n_signal_p = float(allnums[5])
	    n_signal_d = float(allnums[6])
	    n_signal_h = float(allnums[8])
	    n_rate_tot = float(allnums[1])
	    n_rate_p = float(allnums[2])*SF
	    n_rate_d = float(allnums[3])*SF
	    n_rate_h = float(allnums[7])*SF
	    ...
	    RateErr_h = sqrt(float(allnums[7]))*SF
	    """
	    if (len(nsignals_h)<4):
		nsignals_h.append(n_signal_tot)
		nsignals_d.append(n_signal_tot)
		nsignals_p.append(n_signal_tot)
	while npar <4:

	    histname_p = "ratehist_position_eta%dto%d_npar%d_frac%d_pt%d"%(int(etamin*10), int(etamax*10), npar, frac, pt)
	    teffname1_p = "poistioneta%dto%dnpar%dpromptmuonpt%dptbg0fraction%d"%(int(etamin*10), int(etamax*10), npar,pt, frac)
	    histname_d = "ratehist_direction_eta%dto%d_npar%d_frac%d_pt%d"%(int(etamin*10), int(etamax*10), npar, frac, pt)
	    teffname1_d = "directioneta%dto%dnpar%dpromptmuonpt%dptbg0fraction%d"%(int(etamin*10), int(etamax*10), npar,pt, frac)
	    histname_h = "ratehist_hybrid_eta%dto%d_npar%d_frac%d_pt%d"%(int(etamin*10), int(etamax*10), npar, frac, pt)
	    teffname1_h = "hybrideta%dto%dnpar%dpromptmuonpt%dptbg0fraction%d"%(int(etamin*10), int(etamax*10), npar,pt, frac)
	    ...
	    n_rate_p = rfile.Get(histname_p).GetEntries()*SF
	    n_rate_d = rfile.Get(histname_d).GetEntries()*SF
	    n_rate_h = rfile.Get(histname_h).GetEntries()*SF
	    RateErr_p = sqrt(n_rate_p*SF)
	    RateErr_d = sqrt(n_rate_d*SF)
	    RateErr_h = sqrt(n_rate_h*SF)
	    """
...
def CombinedGE21_effAndrate(filename, pts, netas, SBPlots):
	...
	for frac in fractions:
		...
		teff_p_prompt.append(tfile.Get("Teff_prompt_position_eta%dto%d_npar4_frac%d_pt%d"%(int(etamin*10), int(etamax*10), frac, pt)))
		teff_h_prompt.append(tfile.Get("Teff_prompt_hybrid_eta%dto%d_npar4_frac%d_pt%d"%(int(etamin*10), int(etamax*10), frac, pt)))
		teff_h_noGE21_prompt.append(tfile.Get("Teff_prompt_hybrid_eta%dto%d_npar4_frac%d_pt%d_noGE21"%(int(etamin*10), int(etamax*10), frac, pt)))
		...
		histrate_h_noGE21.append(tfile.Get("ratehist_hybrid_eta%dto%d_npar%d_frac%d_pt%d_noGE21"%(int(etamin*10), int(etamax*10), 4, frac, pt)))
	    ...
	    print "pt ",pt," peff ",peff," hnoGE21eff ",hnoGE21eff," heff ",heff," frac ",frac 
	     
	    teff_p_prompt[0].SetName("Teff_prompt_position_eta%dto%d_npar4_frac%d_pt%d_PU%d"%(int(etamin*10), int(etamax*10), frac, pt, pu))
	    teff_h_prompt[0].SetName("Teff_hybrid_position_eta%dto%d_npar4_frac%d_pt%d_PU%d"%(int(etamin*10), int(etamax*10), frac, pt, pu))
	    teff_h_noGE21_prompt[0].SetName("Teff_hybrid_position_eta%dto%d_npar4_frac%d_pt%d_noGE21_PU%d"%(int(etamin*10), int(etamax*10), frac, pt, pu))
	    ...
	    tex2 = TLatex(.15, .75,"p_{T}^{L1}>%d GeV, %.2f<|#eta|<%.2f, overall efficiency %d%%"%(pt, etamin, etamax, frac))
	    tex2.SetNDC()
	    legend2 = ROOT.TLegend(0.15,0.4,0.6,0.6)
...
SBPlots = "eta12to16SBPlots_201700304/"
...
outputroot = "SB_eta12to16_test.root"
getallPtEffAndRate(pts, fractions, netas, outputroot, filedir, condoroutroot, SBPlots, useGE21, True, False, True)
