...
set pythonfile = DisplacedMuon_Efficiency_GEMCSC_position_PU140.py
set runfile = makeplotposition_PU140.sh
set condoroutput = condor20161011_position_PU140
...
cp submit_condor_jobs_position_PU140.cmd $condoroutput
...
#set pt2 = ($Pts_BG[1])
  ...
  cp submit_condor_jobs_position_PU140.cmd submit_condor_jobs_position_copy.cmd
  # changes the the flag FNAME by one of the input 
  # file names in the list
  sed -i "s/PT1/$pt1/g" submit_condor_jobs_position_copy.cmd
  ...
  cat submit_condor_jobs_position_copy.cmd
  condor_submit submit_condor_jobs_position_copy.cmd
#shift Pts_BG
#done
end
