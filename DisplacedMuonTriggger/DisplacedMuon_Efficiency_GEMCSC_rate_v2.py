...
def loopEllipse(filedir, treename, b0, nEvent_list, ddYDPhiCuts, fraction, npar, astart, bstart, xaxis, yaxis,x_bins, y_bins,xtitle, ytitle,st_title, etamin, etamax, cuts,text,picname):
	...
	signalAcceptanceFactor, backgroundRejectionFactor = getBackgroundRejectionEllipse(max_a, max_b, 0, centerx, centery, b1, b0)
        drawEllipse(b1, b0, nEvent_list, ddYDPhiCuts, max_a, max_b, 0, centerx, centery, signalAcceptanceFactor, backgroundRejectionFactor, xtitle, ytitle,st_title, text, picname+"_maxfactor%d"%(int(factor*10)))
    	if signalAcceptanceFactor > fraction+0.02:#make sure fraction<.97
		break
	...
	signalAcceptanceFactor, backgroundRejectionFactor = getBackgroundRejectionEllipse(min_a, min_b, 0, centerx, centery, b1, b0)
        drawEllipse(b1, b0, nEvent_list, ddYDPhiCuts, min_a, min_b, 0, centerx, centery, signalAcceptanceFactor, backgroundRejectionFactor, xtitle, ytitle,st_title, text, picname+"_minfactor%d"%(int(factor*10)))
    ...
    if max_b>1.0:
    	max_b = 1.0
    a_range = frange(min_a, max_a, xbinwidth*2)
    ...
    #alpha_degrees = [0.0, 2, 4, 6, 180-2, 180-4, 180-6]
    ...
    nEvent_list.append(int(nEvent_list[1]*(1-maxRej)))
...
if len(sys.argv)>=4:
    ...
    pt1 = 10
    fraction = 92
    outputdir1 = "Profile_Ellipse_PT_20190304_00_eta18to20_frac92_GE21_test/"
    outputdir2 = "Hybrid_Ellipse_PT_20190304_00_eta18to20_fract92_GE21_test/"
...
def plotalleta(pt, pt1, netas, fraction1 = 90, fraction=95, fraction_d = 99, yaxis = "(csc_bending_angle12_xfactor_L1_2)",ME21CSConly=False):
    ...
    #fraction_d = 99
    GE21string= "withGE21"
    ratetodraw = "dphi_dir12_GE21:ddY123"
    if ME21CSConly:
    	extrapic = "_ME21CSConly"
        extratxt = ", ME21 CSC only"
	GE21string = "NoGE21"
	ratetodraw = "dphi_dir12_noGE21:ddY123"
    ...
    for neta in range(len(netas)-1):
       ...
       for npar in allnpar:
		...
		ratecuts = "maxPositionPt && abs(L1Mu_eta)>%f && abs(L1Mu_eta)<%f && npar==%d"%(etamin, etamax, npar)
    		#h_rate_string = "h_displacedMu_dPhi_ddY_%s_eta%dto%d_npar%d"%(GE21string, int(etamin*10), int(etamax*10),npar)
		h_rate = TH2F("h_rate","rate sample, npar=%d"%npar,2000, -40, 40, 400, -1.0, 1.0)
		...
		ddY_cut = getcuts(filedirs_v6[1], treename, cuts_p[1], xaxis, pt, npar,netas[neta], netas[neta+1], fraction, "(4000,0,40.0)", xtitle,"",outputdir2+"GEMCSC_ctau0_positionOnly_getcuts_recopt_20170131_ptcut%d_st2eta%dto%d_npar%d%s"%(pt, int(netas[neta]*10), int(netas[neta+1]*10), npar,extrapic))
		dphi_dir_cut = getcuts(filedirs_v6[1], treename, cuts_d[1], yaxis, pt, npar,netas[neta], netas[neta+1], fraction, "(3000,0,1.0)", ytitle,"",outputdir2+"GEMCSC_ctau0_directionOnly_getcuts_recopt_20170131_ptcut%d_st2eta%dto%d_npar%d%s"%(pt, int(netas[neta]*10), int(netas[neta+1]*10), npar,extrapic))
		...
		astart = .1
		bstart = .0# not used 
		(maxa, maxb, alpha, x0, y0) = loopEllipse(filedirs_v6, treename, h_rate, nEvent_list, ddYDPhiCuts, fraction, npar, astart, bstart, xaxis, yaxis,x_bins, y_bins,xtitle, ytitle,st_title, netas[neta], netas[neta+1], cuts, text,outputdir1+"GEMCSC_ctau0andctau1000_hyrid_profile_20170131_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d%s"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
...
#plotalleta(pt1,pt2, [2.0, 2.15],90, fraction, 99, "(dphi_dir_st1_st2_L1)",False)
