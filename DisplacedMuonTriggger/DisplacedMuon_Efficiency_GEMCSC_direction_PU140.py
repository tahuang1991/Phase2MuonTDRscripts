...
def DrawProfileAndScatter_pT(filedir,treename,fraction, xaxis,yaxis0, yaxis,x_bins,y_bins0, y_bins,xtitle,ytitle0, ytitle,st_title, etamin, etamax, cuts,text,picname):
    ...
    b0.SetTitle("#scale[1.4]{#font[61]{CMS}} #font[52]{Simulation preliminary}"+"  "*10+" 14 TeV, 140 PU")
...
def printcuts(filedir, treename0, den, num, pt, npar,etamin, etamax, fractionToKeep, xtitle, ytitle, txt, picname, LUT):
    		chain = ROOT.TChain(treename0)
    		if os.path.isdir(filedir):
    			ls = os.listdir(filedir)
    	 		for x in ls:
...
def printallcuts(filedir, treename0, den, num, netas, pts, fractionToKeep, xtitle, ytitle, txt,picname, LUT):
    evenodds = ["odd,even","odd,odd","even,even","even,odd","all pairs"]
    for pt in pts:
		...
		for npar in allnpar:
			...
			den1 = den+"&& fabs(eta_st2_sh)>%f && fabs(eta_st2_sh)<%f && meRing==%d"%(netas[neta], netas[neta+1],ring)
		        printcuts(filedir, treename0, den1, num, pt, npar,etamin,etamax, fractionToKeep, xtitle, ytitle, txt1, picname1, LUT)
...
def makeEffplot_v2(filedirs,todraw, treename0, den, num, pt, allnpar, fractionToKeep,  xtitle,ytitle,leg, txt,picname):
	...
	legend = ROOT.TLegend(0.4,0.15,0.8,0.5)
	legend.SetFillColor(ROOT.kWhite)
	legend.SetTextFont(42)
...
LUT = open("Directioni_LUT_PU140_1010.log","w+")
printallcuts(filedir16, treename0,"hasSt3orSt4_sh", "csc_bending_angle12_xfactor_L1_2", netas, [2,3,4,5,7,10,15,20,30,40], 90, "|#Delta#phi_{dir}|", "Entries", "digi level ","Direction_PT_Cut_0909/GEMCSC_ctau0_directionOnly_20160909", LUT)
...
#plotalleta(pt,[2.0,2.2],90,"(csc_bending_angle12_xfactor_L1_2)",False)
plotalleta(pt,[1.6,1.8,2.0,2.2],90,"(csc_bending_angle12_xfactor_L1_2)",False)
plotalleta(pt,[1.6,1.8,2.0,2.2],90,"(csc_bending_angle12_xfactor_L1_1)",True)
plotalleta(pt,[1.2,1.4,1.6],90,"(csc_bending_angle12_xfactor_L1_2)",False)
