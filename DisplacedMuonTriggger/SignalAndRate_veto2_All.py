...
#ratesample = "RateTree_GE11GE21_displacedmu_noveto_ME0eta215.root"
...
#signalsampleGE11 = "/Users/taohuang/Documents/CSCTrigger/GEMCSCbending_20170620_PU200/SingleMu_EMTF_ME0_PU200_GEMCSCANA_20170722_GE11only.root"
...
#signalsampleGE11 = "GEMCSC_Ana_ctau0_Pt2_50_PU140_20170712_GE11only/out_ana_1.root"
...
#ratesample = "RateTree_20170627.root" #ME0Eta2.15
#ratesample = "RateTree_20170613_Q4.root"
#ratesample = "RateTree_20170620_ME0Eta21.root"

#signalsample = "out_ana_displaced_singleGE21.root"
#signalsample = "out_ana_displacedMu_PU0_192_20170609.root"
#signalsample = "out_ana_displacedMu_PU0_192_20170612.root"
#signalsample = "GEMCSCAna_DisplacedMu_PU0_192_20170613/"
#signalsample = "GEMCSCAna_DisplacedMu_PU0_192_20170625_eta0_28/"
...
color = [ROOT.kBlue,ROOT.kBlack, ROOT.kGreen+2, ROOT.kRed, ROOT.kOrange+8, ROOT.kBlack]
maker =[25, 21,25,20,22]
...
def makeEffplot_v2(chainlist,todraw, den, num, etamin, etamax, xtitle,ytitle,leg, legheader, txt,picname):
    ...
    color = [ROOT.kBlack, ROOT.kBlue,  ROOT.kRed, ROOT.kGreen+2, kBlack]
    marker = [21, 25, 26, 20, 24]
...
def plotshistograms(hists, legs,legheader, text, picname, RateVsPt=False):
    ...
    legend = ROOT.TLegend(0.16,0.49,0.65,0.70+dy_leg)
...
def plotalleta(pt, netas, fraction, outputrootfile, ratesample, signalsample, allLUTfile, SBPlots):
    ...
    for neta in range(len(netas)-1):
       ...
       for npar in range(0, 4):
	    ...
	    #cutsbase = " meRing==%d && "%ring+"fabs(eta_st2_L1)>%f && fabs(eta_st2_L1)<%f && fabs(genGdMu_dxy)>10 && fabs(genGdMu_dxy)<50 && fabs(genGdMu_dR)<0.1 && genGdMu_lxy < 500 "%(etamin,etamax)+L1Mucut+"&&"+simnpar
	    ...
	    print "baseratecut ",ratecut,"&& abs(%s)<=%f"%(rate_x, ddY_cut)," ratehist_p_veto0 ",ratehist_p_veto0.GetEntries()
    ...
    makeplotsEta([Teffalleta0_p,  Teffalleta0_h], legs2, legheader,text_noveto,SBPlots+"GEMCSC_dxy10_50_combined3_noveto_effvseta_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(etamin*10),int(etamax*10))) 
    makeplotsEta([Teffalleta2_p,  Teffalleta2_h], legs2, legheader,text_mveto,SBPlots+"GEMCSC_dxy10_50_combined3_mediumveto_effvsetaf_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(etamin*10),int(etamax*10))) 
    makeplots([Teffall0_p, Teffall0_h], legs2, legheader,text_noveto,SBPlots+"GEMCSC_dxy10_50_combined3_noveto_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(etamin*10),int(etamax*10))) 
    makeplots([Teffall2_p, Teffall2_h], legs2, legheader,text_mveto,SBPlots+"GEMCSC_dxy10_50_combined3_mediumveto_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(etamin*10),int(etamax*10))) 
    plotshistograms([hrateall0_p, hrateall0_h], legs2, legheader, text_noveto, SBPlots+"TriggerRate_combiend_noveto_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(etamin*10),int(etamax*10)))
    plotshistograms([hrateall2_p, hrateall2_h], legs2, legheaderm, text_mveto, SBPlots+"TriggerRate_combiend_mediumveto_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(etamin*10),int(etamax*10)))
...
def plotalletaME0GE21(pt, netas, fraction, outputrootfile, ratesample, signalsample, allLUTfile, SBPlots):
    ...
    for neta in range(len(netas)-1):
       ...
       for npar in range(0, 4):
	    ...
	    dphi_cut1 = getCut(GEMCSCTrackCh0, "abs("+ yaxis1 +")", cuts_d[0]+"&& pt>%f"%(pt), 0.0, 1.5, 0.005, fraction*1.0/100.0)
	    dphi_cut2 = getCut(GEMCSCTrackCh0, "abs("+yaxis2+")", cuts_d[0]+"&& pt>%f"%(pt), 0.0, 1.5, 0.005, fraction*1.0/100.0)
	    dphi_cut3 = getCut(GEMCSCTrackCh0, "abs("+yaxis3+")", cuts_d[0]+"&& pt>%f"%(pt), 0.0, 1.5, 0.005, fraction*1.0/100.0)
	    #ddY_cut2 = getCut(GEMCSCTrackCh0, "abs("+xaxis+")", cuts_p[0]+"&& pt>%f"%(pt), 0.0, 10, 0.01, 0.99)
	    ddY_cut2 = 10
	   
	    #ddY_cut = getCut(GEMCSCTrackCh0, "abs("+xaxis+")", cuts_p[0]+"&& pt>%f"%(pt), 0.0, 40, 0.01, fraction*1.0/100.0)
	    ...
	    ratehist_h2_veto0 = getRateVsEta(L1MuTriggerRate, "ratehist_hybrid1_eta%dto%d_npar%d_frac%d_pt%d"%(int(etamin*10),int(etamax*10), npar, fraction, pt), myetabin, ratecut + L1Mucut + "&& abs(%s)<=%f"%(rate_y2, dphi_cut2)+ "&& abs(%s)<=%f"%(rate_x, ddY_cut2), 0)
	    ratehist_h3_veto0 = getRateVsEta(L1MuTriggerRate, "ratehist_hybrid1_eta%dto%d_npar%d_frac%d_pt%d"%(int(etamin*10),int(etamax*10), npar, fraction, pt), myetabin, ratecut + L1Mucut + "&& abs(%s)<=%f"%(rate_y3, dphi_cut3)+ "&& abs(%s)<=%f"%(rate_x, ddY_cut2), 0)
	    #ratehist_p_veto2 = getRateVsEta(L1MuTriggerRate, "ratehist_position_mediumveto_eta%dto%d_npar%d_frac%d_pt%d"%(int(etamin*10),int(etamax*10), npar, fraction, pt), myetabin, ratecut + "&& abs(%s)<=%f"%(rate_x, ddY_cut), 2)
       ...
       makeplots([Teff0_p, Teff0_h1, Teff0_h2, Teff0_h3], legsall, legheader, text_all, SBPlots+"GEMCSC_dxy5_25_combined3_noveto_eff_20170611_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(etamin*10),int(etamax*10))) 
    ...
    legheader = "L1Mu with Q>=%d, no track veto"%(quality)
    legheaderm = "L1Mu with medium track veto, Q>=%d"%(quality)
    legs2all = ["CSC+GE11: position based","CSC+GE11+GE21+ME0: hybrid algorithm"]
    makeplots([Teffall0_p, Teffall0_h3], legs2all, legheader,text_all,SBPlots+"GEMCSC_dxy5_25_combined3_noveto_eff_v2_20170611_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(etamin*10),int(etamax*10))) 
    makeplots([Teffall0_p, Teffall0_h1, Teffall0_h2 ,Teffall0_h3], legsall, legheader,text_all,SBPlots+"GEMCSC_dxy5_25_combined3_noveto_eff_20170611_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(etamin*10),int(etamax*10))) 
    makeplotsEta([Teffalleta0_p, Teffalleta0_h3], legs2all, legheader,text_all,SBPlots+"GEMCSC_dxy5_25_combined3_noveto_effvseta_v2_20170611_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(etamin*10),int(etamax*10))) 
    ...
    #makeplots([Teffall2_p, Teffall2_h_noGE21, Teffall2_h], legs3, legheaderm,text_mveto,SBPlots+"GEMCSC_dxy10_50_combined3_mediumveto_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(etamin*10),int(etamax*10))) 
...
#SBPlots = "eta12to215SBPlots_20170724_combined_PU200_TriggerRateVsEta_ME0eta215/"
SBPlots = "eta12to21SBPlots_20171106_PU200_TriggerRateVsPT/"
#SBPlots = "eta21to24SBPlots_20170724_combined_PU200_TriggerRateVsEta_combined/"
#SBPlots = "eta21to24SBPlots_20170724_combined_PU200_TriggerRateVsEta_Q4/"
#SBPlots = "eta165to21SBPlots_20170724_combined_PU200_TriggerRateVsEta_ME0eta215/"
if len(sys.argv)>=2:
        SBPlots = sys.argv[1]
	if SBPlots[-1] != "/":
	    SBPlots = SBPlots+"/"
