...
def plotshistograms(hists, legs, text, picname):
...
def plotalleta(pt, netas, fraction, outputrootfile, ratesample, signalsample, allLUTfile):
    #fraction_d = 99
    ...
    for neta in range(len(netas)-1):
       ...
       for npar in range(0, 4):
	    ...
	    cutsbase = "L1Mu_quality>=4 && meRing==%d && "%ring+"hasSt1St2St3 && fabs(eta_st2_L1)>%f && fabs(eta_st2_L1)<%f && fabs(genGdMu_dxy)>10 && fabs(genGdMu_dxy)<50 && fabs(genGdMu_dR)<0.1 && npar_L1==%d"%(netas[neta],netas[neta+1], npar)
	    ...
	    text = "#splitline{%s}{%.1f<|#eta|<%.2f, p_{T}^{L1}>%d GeV}"%(chambers, netas[neta],netas[neta+1],pt)
	    text_position = "#splitline{%s}{%.1f<|#eta|<%.2f, p_{T}^{L1}>%d GeV}"%(chambers,netas[neta],netas[neta+1],pt)
	    #ddY_cut = getcuts(GEMCSCTrackCh1, cuts_p[1], xaxis, pt, npar,netas[neta], netas[neta+1], fraction, "(4000,0,40.0)", xtitle,"",SBPlots+"GEMCSC_ctau0_positionOnly_getcuts_recopt_20170131_ptcut%d_st2eta%dto%d_npar%d%s"%(pt, int(netas[neta]*10), int(netas[neta+1]*10), npar,extrapic))
	    #dphi_dir_cut = getcuts(GEMCSCTrackCh1, cuts_d[1], yaxis, pt, npar,netas[neta], netas[neta+1], fraction, "(3000,0,1.0)", ytitle,"",SBPlots+"GEMCSC_ctau0_directionOnly_getcuts_recopt_20170131_ptcut%d_st2eta%dto%d_npar%d%s"%(pt, int(netas[neta]*10), int(netas[neta+1]*10), npar,extrapic))
	    #ddYDPhiCuts = [ddY_cut, dphi_dir_cut]
	    
	    #Teffs_ddY = makeEffplot_v2([GEMCSCTrackCh0], "pt", cuts_p, ["fabs(%s)<%f"%(xaxis, ddY_cut)], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs,"Position-based algo" ,text_position,SBPlots+"GEMCSC_dxy10_50_position_eff_20170131_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
	    #Teffs_dphi_dir = makeEffplot_v2([GEMCSCTrackCh0], "pt", cuts_d, ["fabs(%s)<%f"%(yaxis, dphi_dir_cut)], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs,"Stub alignment algo",text,SBPlots+"GEMCSC_dxy10_50_direction_eff_20170131_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
	    ...
	    ratehist_p_veto0 = getRateVsEta(L1MuTriggerRate, "ratehist_position_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt), myetabin, ratecut + "&& abs(%s)<=%f"%(rate_x, ddY_cut), 0)
	    ratehist_h_veto0 = getRateVsEta(L1MuTriggerRate, "ratehist_hybrid_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt), myetabin, ratecut + "&& "+ellipsecut_rate, 0)
	    ratehist_h_noGE21_veto0 = getRateVsEta(L1MuTriggerRate, "ratehist_hybrid_noGE21_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt), myetabin, ratecut + "&& "+ellipsecut_rate_noGE21, 0)
	    ratehist_p_veto2 = getRateVsEta(L1MuTriggerRate, "ratehist_position_mediumveto_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt), myetabin, ratecut + "&& abs(%s)<=%f"%(rate_x, ddY_cut), 2)
	    ratehist_h_veto2 = getRateVsEta(L1MuTriggerRate, "ratehist_hybrid_mediumveto_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt), myetabin, ratecut + "&& "+ellipsecut_rate, 2)
	    ratehist_h_noGE21_veto2 = getRateVsEta(L1MuTriggerRate, "ratehist_hybrid_noGE21_mediumveto_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt), myetabin, ratecut + "&& "+ellipsecut_rate_noGE21, 2)
	    ...
	    Teffs = makeEffplot_v2([GEMCSCTrackCh0, GEMCSCTrackCh0, GEMCSCTrackCh0], "pt", [cutsbase+checkx, cutsbase+checkx+checky_noGE21, cutsbase+checkx+checky], ["fabs(%s)<%f"%(xaxis, ddY_cut), ellipse_noGE21, ellipse], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs3,"no tracker veto",text,SBPlots+"GEMCSC_dxy10_50_combined_eff_20170131_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
	    Teffs_veto2 = makeEffplot_v2([GEMCSCTrackCh0, GEMCSCTrackCh0, GEMCSCTrackCh0], "pt", [cutsbase+checkx, cutsbase+checkx+checky_noGE21, cutsbase+checkx+checky], ["fabs(%s)<%f"%(xaxis, ddY_cut)+"&& !isL1MediumVeto", ellipse_noGE21+"&& !isL1MediumVeto", ellipse+"&& !isL1MediumVeto"], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs3,"medium tracker veto",text,SBPlots+"GEMCSC_dxy10_50_mediumveto_combined_eff_20170131_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
	    #Teffs_veto2 = makeEffplot_v2([GEMCSCTrackCh0, GEMCSCTrackCh0, GEMCSCTrackCh0], "pt", [cutsbase+checkx, cutsbase+checkx+checky_noGE21, cutsbase+checkx+checky], ["fabs(%s)<%f"%(xaxis, ddY_cut)+"&& isSimMediumVeto", ellipse_noGE21+"&& isSimMediumVeto", ellipse+"&& isSimMediumVeto"], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs3,"medium tracker veto",text,SBPlots+"GEMCSC_dxy10_50_mediumveto_combined_eff_20170131_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
	    ...
	    #makeplots([Teffs_ddY[0], Teffs_dphi_dir[0], Teffs[0]], legs3, text,SBPlots+"GEMCSC_ctau0andctau1000_combined3_eff_20170131_pt%d_ptbg%d_fraction%d_fractiond%d_St2eta%dto%d_npar%d_dirinterstation%s"%(pt, pt1, fraction, fraction_d, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic)) 
	    ...
	    Teffs[0].SetName("positioneta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"displacedmuonpt%d"%(pt)+"fraction%d"%fraction)
	    Teffs[2].SetName("hybrideta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"displacedmuonpt%d"%(pt)+"fraction%d"%fraction)
	    Teffs[1].SetName("hybrid_noGE21eta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"displacedmuonpt%d"%(pt)+"fraction%d"%fraction)
	    Teffs_veto2[0].SetName("positionmediumvetoeta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"displacedmuonpt%d"%(pt)+"fraction%d"%fraction)
	    Teffs_veto2[2].SetName("hybridmediumvetoeta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"displacedmuonpt%d"%(pt)+"fraction%d"%fraction)
	    Teffs_veto2[1].SetName("hybrid_noGE21mediumvetoeta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"displacedmuonpt%d"%(pt)+"fraction%d"%fraction)
       ...
       Teff0_p.SetName("positioneta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"displacedmuonpt%d"%(pt))
       Teff0_h.SetName("hybrideta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"displacedmuonpt%d"%(pt))
       Teff0_h_noGE21.SetName("hybrid_noGE21eta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"displacedmuonpt%d"%(pt))
       Teff2_p.SetName("positionmediumvetoeta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"displacedmuonpt%d"%(pt))
       Teff2_h.SetName("hybridmediumvetoeta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"displacedmuonpt%d"%(pt))
       Teff2_h_noGE21.SetName("hybrid_noGE21mediumvetoeta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"displacedmuonpt%d"%(pt))
       Teff0_p.Write(); Teff0_h.Write(); Teff0_h_noGE21.Write()
       Teff2_p.Write(); Teff2_h.Write(); Teff2_h_noGE21.Write()
       text_h = "#splitline{Hybrid algorithm w/ GE21}{%.2f<|#eta|<%.2f, p_{T}^{L1}>%d GeV}"%(netas[neta],netas[neta+1], pt)
       text_p = "#splitline{Position-based algorithm}{%.2f<|#eta|<%.2f, p_{T}^{L1}>%d GeV}"%(netas[neta],netas[neta+1], pt)
       text_h_noGE21 = "#splitline{Hybrid algorithm w/o GE21}{%.2f<|#eta|<%.2f, p_{T}^{L1}>%d GeV}"%(netas[neta],netas[neta+1], pt)
       legsveto = ["no tracker veto","medium tracker veto"]
       makeplots([Teff0_p, Teff2_p], legsveto, text_p,SBPlots+"GEMCSC_dxy10_50_position_vetos_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10))) 
       makeplots([Teff0_h, Teff2_h], legsveto, text_h,SBPlots+"GEMCSC_dxy10_50_hybrid_vetos_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10))) 
       makeplots([Teff0_h_noGE21, Teff2_h_noGE21], legsveto, text_h_noGE21,SBPlots+"GEMCSC_dxy10_50_hybrid_noGE21_vetos_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10))) 
       plotshistograms([hrate0_p, hrate2_p], legsveto, text_p, SBPlots+"TriggerRate_position_vetos_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10)))
       plotshistograms([hrate0_h, hrate2_h], legsveto, text_h, SBPlots+"TriggerRate_hybrid_vetos_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10)))
       plotshistograms([hrate0_h_noGE21, hrate2_h_noGE21], legsveto, text_h_noGE21, SBPlots+"TriggerRate_hybrid_noGE21_vetos_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10)))
       text_noveto =  "#splitline{%.2f<|#eta|<%.2f, p_{T}^{L1}>%d GeV}{no tracker veto}"%(netas[neta],netas[neta+1], pt)
       text_mveto =  "#splitline{%.2f<|#eta|<%.2f, p_{T}^{L1}>%d GeV}{medium tracker veto}"%(netas[neta],netas[neta+1], pt)
       makeplots([Teff0_p, Teff0_h_noGE21, Teff0_h], legs3, text_noveto,SBPlots+"GEMCSC_dxy10_50_combined3_noveto_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10))) 
       makeplots([Teff2_p, Teff2_h_noGE21, Teff2_h], legs3, text_mveto,SBPlots+"GEMCSC_dxy10_50_combined3_mediumveto_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10))) 
       plotshistograms([hrate0_p, hrate0_h_noGE21, hrate0_h], legs3, text_noveto, SBPlots+"TriggerRate_combiend_noveto_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10)))
       plotshistograms([hrate2_p, hrate2_h_noGE21, hrate2_h], legs3, text_mveto, SBPlots+"TriggerRate_combiend_mediumveto_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10)))
    etamin = netas[0]; etamax = netas[-1]
    ...
    peff0, perr0 = getOverallEff(Teffall0_p, pt)
    peff2, perr2 = getOverallEff(Teffall2_p, pt)
    heff0_noGE21, herr0_noGE21 = getOverallEff(Teffall0_h_noGE21, pt)
    heff2_noGE21, herr2_noGE21 = getOverallEff(Teffall2_h_noGE21, pt)
    heff0, herr0 = getOverallEff(Teffall0_h, pt)
    heff2, herr2 = getOverallEff(Teffall2_h, pt)
    print "pt ",pt," fraction ",fraction," etamin ",etamin, " etamax ",etamax," allnpar peff0 ",peff0, " heff0 ",heff0," heff0_noGE21 ",heff0_noGE21
    print "Rate entreis p ",hrate0_p.GetEntries()," h_noGE21 ",hrate0_h_noGE21.GetEntries()," h ",hrate0_h.GetEntries()
    print "all eff, medium veto p ",peff2," heff2_noGE21 ",heff2_noGE21," heff2 ",heff2
    print "Rate, medium veto,  entreis p ",hrate2_p.GetEntries()," h_noGE21 ",hrate2_h_noGE21.GetEntries()," h ",hrate2_h.GetEntries()
    Teffall0_p.SetName("positioneta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"displacedmuonpt%d"%(pt))
    Teffall0_h.SetName("hybrideta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"displacedmuonpt%d"%(pt))
    Teffall0_h_noGE21.SetName("hybrid_noGE21eta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"displacedmuonpt%d"%(pt))
    Teffall2_p.SetName("positionmediumvetoeta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"displacedmuonpt%d"%(pt))
    Teffall2_h.SetName("hybridmediumvetoeta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"displacedmuonpt%d"%(pt))
    Teffall2_h_noGE21.SetName("hybrid_noGE21mediumvetoeta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"displacedmuonpt%d"%(pt))
    Teffall0_p.Write(); Teff0_h.Write(); Teff0_h_noGE21.Write()
    Teffall2_p.Write(); Teff2_h.Write(); Teff2_h_noGE21.Write()
    hrate0_p.SetName()
    text_h = "#splitline{Hybrid algorithm w/ GE21}{%.2f<|#eta|<%.2f, p_{T}^{L1}>%d GeV}"%(netas[neta],netas[neta+1], pt)
    text_p = "#splitline{Position-based algorithm}{%.2f<|#eta|<%.2f, p_{T}^{L1}>%d GeV}"%(netas[neta],netas[neta+1], pt)
    text_h_noGE21 = "#splitline{Hybrid algorithm w/o GE21}{%.2f<|#eta|<%.2f, p_{T}^{L1}>%d GeV}"%(netas[neta],netas[neta+1], pt)
    legsveto = ["no tracker veto","medium tracker veto"]
    makeplots([Teffall0_p, Teffall2_p], legsveto, text_p,SBPlots+"GEMCSC_dxy10_50_position_vetos_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10))) 
    makeplots([Teffall0_h, Teffall2_h], legsveto, text_h,SBPlots+"GEMCSC_dxy10_50_hybrid_vetos_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10))) 
    makeplots([Teffall0_h_noGE21, Teffall2_h_noGE21], legsveto, text_h_noGE21,SBPlots+"GEMCSC_dxy10_50_hybrid_noGE21_vetos_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10))) 
    plotshistograms([hrateall0_p, hrateall2_p], legsveto, text_p, SBPlots+"TriggerRate_position_vetos_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10)))
    plotshistograms([hrateall0_h, hrateall2_h], legsveto, text_h, SBPlots+"TriggerRate_hybrid_vetos_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10)))
    plotshistograms([hrateall0_h_noGE21, hrateall2_h_noGE21], legsveto, text_h_noGE21, SBPlots+"TriggerRate_hybrid_noGE21_vetos_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10)))
    text_noveto =  "#splitline{%.2f<|#eta|<%.2f, p_{T}^{L1}>%d GeV}{no tracker veto}"%(netas[neta],netas[neta+1], pt)
    text_mveto =  "#splitline{%.2f<|#eta|<%.2f, p_{T}^{L1}>%d GeV}{medium tracker veto}"%(netas[neta],netas[neta+1], pt)
    makeplots([Teffall0_p, Teffall0_h_noGE21, Teffall0_h], legs3, text_noveto,SBPlots+"GEMCSC_dxy10_50_combined3_noveto_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10))) 
    makeplots([Teffall2_p, Teffall2_h_noGE21, Teffall2_h], legs3, text_mveto,SBPlots+"GEMCSC_dxy10_50_combined3_mediumveto_eff_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10))) 
    plotshistograms([hrateall0_p, hrateall0_h_noGE21, hrateall0_h], legs3, text_noveto, SBPlots+"TriggerRate_combiend_noveto_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10)))
    plotshistograms([hrateall2_p, hrateall2_h_noGE21, hrateall2_h], legs3, text_mveto, SBPlots+"TriggerRate_combiend_mediumveto_20170131_pt%d_fraction%d_St2eta%dto%d_allnpar"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10)))
    Teff_out.Close()
#DecompresZipFiles("./")
#SBPlots = "eta16to215SBPlots_20170304_withGE21_v2/"
#SBPlots = "eta16to215SBPlots_20170304_combined_PU200/"
SBPlots = "eta12to215SBPlots_20170306_combined_PU140_test/"
...
filename = "SB_eta12to215_all_20170306_backup.root"
filename = "SB_eta12to215_all_20170306.root"
filename = "SB_eta12to16_20170304_test.root"
...
outputrootfile = "SB_eta16to215_20170315_test.root"
fraction = 92
netas = [1.2,1.4,1.6,1.8,2.0,2.15]
ratesample = "RateTree.root"
signalsample = "out_ana_displaced.root"
plotalleta(10, netas, fraction, outputrootfile, ratesample, signalsample, allLUTfile)
