...
def loopEllipse_trainingwithPromptMu(filedir, treename,fraction, astart, bstart, xaxis, yaxis,x_bins, y_bins,xtitle, ytitle,st_title, etamin, etamax, cuts,text,picname):
...
def getcuts(filedir, treename0, den, num, pt, npar,etamin, etamax, fractionToKeep,x_bins, xtitle, txt, picname):
	...
	if pt>2 and pt<10:
	   	...
	   	deltapt=1.0	   
        chain.Draw("TMath::Abs(%s)>>hist"%(num),den+"&& npar==%d && pt>%f && pt<%f"%(npar,pt-deltapt, pt+deltapt))
       	#hist.Print("ALL")
	upperlim = getUpperlimit(hist, fractionToKeep)
       	print "cuts ",den+"&& npar==%d && pt>%f && pt<%f"%(npar,pt-1, pt+1)," upperlim ",upperlim
        return upperlim
        """
...
filedir46 = "/eos/uscms/store/user/tahuang/DarkSUSY_ctau1000_GEMCSCAna_20170530/"
...
if len(sys.argv)>=5:
	...
	#	outputdir2 = outputdir2+"/"
        ...
        pt1=19; pt2=5
	outputdir1 = "Profile_Ellipse_PT_20190304_00_norotation/"
	outputdir2 = "Hybrid_Ellipse_PT_20190304_00_norotation/"
...
def plotalleta(pt, pt1, netas, fraction1 = 90, fraction=95, fraction_d = 99, yaxis = "(csc_bending_angle12_xfactor_L1_2)",ME21CSConly=False):
    ...
    LUT = open("HybridLUT_Pt%d_BGPt%d_eta%dto%d%s_0929.log"%(pt,pt1,int(netas[0]*10), int(netas[-1]*10), extrapic),"w+")
    ...
    for neta in range(len(netas)-1):
       ...
       for npar in allnpar:
		...
		#yaxis = "(csc_bending_angle12_xfactor_L1_2)"#deltaphi, only in high eta region, CSC only
    		...
    		checkvalue = "&& fabs(deltay23_fit)<50 && fabs(deltay12_fit)<50 && fabs(%s)>0 && fabs(%s)>0 && meRing==%d"%(xaxis, yaxis,ring)
		...
		cuts =  ["meRing==%d && "%ring+"hasSt1St2St3 && fabs(eta_st2_sh)>%f && fabs(eta_st2_sh)<%f && npar==%d"%(netas[neta],netas[neta+1], npar)+checkx+checky, "meRing==%d && "%ring+"hasSt1St2St3 && fabs(eta_st2_sh)>%f && fabs(eta_st2_sh)<%f && fabs(genGdMu_dxy)>10 && fabs(genGdMu_dxy)<50 && fabs(genGdMu_dR)<0.1 && npar==%d"%(netas[neta],netas[neta+1], npar)+checkx+checky]
		cuts_p =  ["meRing==%d && "%ring+" hasSt1St2St3 && fabs(eta_st2_sh)>%f && fabs(eta_st2_sh)<%f && npar==%d"%(netas[neta],netas[neta+1], npar)+checkx, "meRing==%d && "%ring+"hasSt1St2St3 && fabs(eta_st2_sh)>%f && fabs(eta_st2_sh)<%f && fabs(genGdMu_dxy)>10 && fabs(genGdMu_dxy)<50 && fabs(genGdMu_dR)<0.1 && npar==%d"%(netas[neta],netas[neta+1], npar)+checkx]
		...
		ddY_cut = getcuts(filedirs_v6[0], treename, cuts_p[0], xaxis, pt, npar,netas[neta], netas[neta+1], fraction1, "(4000,0,40.0)", xtitle,"",outputdir2+"GEMCSC_ctau0_positionOnly_getcuts_recopt_20160929_ptcut%d_st2eta%dto%d_npar%d%s"%(pt, int(netas[neta]*10), int(netas[neta+1]*10), npar,extrapic))
		dphi_dir_cut = getcuts(filedirs_v6[0], treename, cuts_d[0], yaxis, pt, npar,netas[neta], netas[neta+1], fraction1, "(3000,0,1.0)", ytitle,"",outputdir2+"GEMCSC_ctau0_directionOnly_getcuts_recopt_20160929_ptcut%d_st2eta%dto%d_npar%d%s"%(pt, int(netas[neta]*10), int(netas[neta+1]*10), npar,extrapic))
		Teffs_ddY = makeEffplot_v2(filedirs_v6, "pt", treename, cuts_p, ["fabs(%s)<%f"%(xaxis, ddY_cut), "fabs(%s)<%f"%(xaxis, ddY_cut)], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs,"Position-based algo" ,text_position,outputdir2+"GEMCSC_ctau0andctau1000_position_eff_20160929_pt%d_fraction%d_st2eta%dto%d_npar%d%s"%(pt, fraction1, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
		Teffs_dphi_dir = makeEffplot_v2(filedirs_v6, "pt", treename, cuts_d, ["fabs(%s)<%f"%(yaxis, dphi_dir_cut), "fabs(%s)<%f"%(yaxis, dphi_dir_cut)], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs,"Stub alignment algo",text,outputdir2+"GEMCSC_ctau0andctau1000_direction_eff_20160929_pt%d_fraction%d_st2eta%dto%d_npar%d%s"%(pt, fraction1, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
		...
		Teffs = makeEffplot_v2(filedirs_v6, "pt", treename, cuts, [ellipse, ellipse], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs,"Hybrid algo",text,outputdir2+"GEMCSC_ctau0andctau1000_hybrid_eff_20160929_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d%s"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
		...
		Teffs = makeEffplot_v2(filedirs_v6, "pt", treename, cuts, [ellipse+"&&"+ellipse_d, ellipse+"&&"+ellipse_d], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs,"Hybrid algo",text,outputdir2+"GEMCSC_ctau0andctau1000_hybrid_eff_20160929_pt%d_ptbg%d_fraction%d_fractiond%d_st2eta%dto%d_npar%d_dirinterstation%s"%(pt, pt1, fraction, fraction_d, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
       		makeplots([Teffs_ddY[0], Teffs_dphi_dir[0], Teffs[0]], legs3, text,outputdir2+"GEMCSC_ctau0andctau1000_combined3_eff_20160929_pt%d_ptbg%d_fraction%d_fractiond%d_St2eta%dto%d_npar%d_dirinterstation%s"%(pt, pt1, fraction, fraction_d, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic)) 
       ...
       makeplots([Teff0_d, Teff1_d], legs, text_d,outputdir2+"GEMCSC_ctau0andctau1000_direction_eff_20160929_pt%d_fraction%d_St2eta%dto%d_allnpar_dirinterstation%s"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), extrapic)) 
       makeplots([Teff0_p, Teff1_p], legs, text_p,outputdir2+"GEMCSC_ctau0andctau1000_position_eff_20160929_pt%d_fraction%d_St2eta%dto%d_allnpar%s"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), extrapic)) 
       text3 = "#splitline{%.1f<|#eta|<%.1f, p_{T}^{L1}>%d GeV%s}{Prompt muon}"%(netas[neta],netas[neta+1], pt, extratxt)
       makeplots([Teff0_p, Teff0_d, Teff0], legs3, text3,outputdir2+"GEMCSC_ctau0andctau1000_combined3_eff_20160929_pt%d_ptbg%d_fraction%d_fractiond%d_St2eta%dto%d_allnpar_dirinterstation%s"%(pt, pt1, fraction, fraction_d, int(netas[neta]*10),int(netas[neta+1]*10), extrapic)) 
       """
    ...
    makeplots([Teffs0_alleta, Teffs1_alleta], legs, text_alleta,outputdir2+"GEMCSC_ctau0andctau1000_hybrid_eff_20160929_pt%d_ptbg%d_fraction%d_fractiond%d_St2eta%dto%d_allnpar_dirinterstation%s"%(pt, pt1, fraction,fraction_d, int(netas[0]*10),int(netas[-1]*10), extrapic)) 
    makeplots([Teffs0_alleta_p, Teffs0_alleta_d, Teffs0_alleta], legs3, text_alleta_3,outputdir2+"GEMCSC_ctau0andctau1000_combined3_eff_20160929_pt%d_ptbg%d_fraction%d_fractiond%d_St2eta%dto%d_allnpar_dirinterstation%s"%(pt, pt1, fraction,fraction_d, int(netas[0]*10),int(netas[-1]*10), extrapic)) 
    """
    Teff_out.Reopen("Update")
...
#plotalleta(pt1,pt2, [1.6,1.8,2.0,2.2],90, 95, 99, "(csc_bending_angle12_xfactor_L1_2)",False)
