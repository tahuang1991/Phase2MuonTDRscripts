...
def plotsgraphs(graphs, legs, legheader, text, picname):
    ...
    legend = ROOT.TLegend(0.15,0.7,0.5,0.7+dy_leg)
    legend.SetFillColor(ROOT.kWhite)
    legend.SetHeader(legheader)
    legend.SetTextFont(42)
    #legend.SetTextSize(.05)
...
def plotalleta(pt, netas, fraction, outputrootfile, ratesample, signalsample, allLUTfile, SBPlots):
    ...
    for neta in range(len(netas)-1):
       ...
       for npar in range(0, 4):
	    ...
	    print "pt ",pt," fraction ",fraction," etamin ",etamin, " etamax ",etamax," npar ",npar, "peff ",peff, " heff ",heff," heff_noGE21 ",heff_noGE21," after veto entries p ",ratehist_p_veto2.GetEntries()," h ", ratehist_h_veto2.GetEntries()," h_noGE21 ",ratehist_h_noGE21_veto2.GetEntries()
	...
	#print "hrateall2_h.GetEntries() ",hrateall2_h.GetEntries()
    #checkout plateau eff
    ...
    hrateall0_p.SetName("ratehist_position_eta%dto%d_allnpar_frac%d_pt%d"%(int(etamin*10),int(etamax*10), fraction, pt))
    hrateall2_p.SetName("ratehist_position_mediumveto_eta%dto%d_allnpar_frac%d_pt%d"%(int(etamin*10),int(etamax*10), fraction, pt))
    hrateall0_h_noGE21.SetName("ratehist_hybrid_noGE21_eta%dto%d_allnpar_frac%d_pt%d"%(int(etamin*10),int(etamax*10), fraction, pt))
    hrateall2_h_noGE21.SetName("ratehist_hybrid_noGE21_mediumveto_eta%dto%d_allnpar_frac%d_pt%d"%(int(etamin*10),int(etamax*10), fraction, pt))
    hrateall0_h.SetName("ratehist_hybrid_eta%dto%d_allnpar_frac%d_pt%d"%(int(etamin*10),int(etamax*10), fraction, pt))
    hrateall2_h.SetName("ratehist_hybrid_mediumveto_eta%dto%d_allnpar_frac%d_pt%d"%(int(etamin*10),int(etamax*10), fraction, pt))
...
outputrootfile = "SB_eta16to215_20170316_test.root"
