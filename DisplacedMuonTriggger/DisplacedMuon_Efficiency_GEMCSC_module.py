...
filedir46 = "/eos/uscms/store/user/tahuang/DarkSUSY_MH-125_MGammaD-20000_ctau100_14TeV_madgraph-pythia6-tauola/GEMCSCAna_DarkSUSY_ctau100_20161218/161219_063009/0000/"
...
if len(sys.argv)>=5:
	...
	pt1 = 10
	pt2 = 7
	outputdir1 = "Profile_Ellipse_PT_20170208_00_test/"
	outputdir2 = "Hybrid_Ellipse_PT_20170208_00_test/"

LUT = open("HybridLUT_Pt%d_BGPt%d_20170131.log"%(pt1, pt2),"w+")
...
def plotalleta(pt, pt1, netas, fraction1 = 90, fraction=95, fraction_d = 99, yaxis = "(csc_bending_angle12_xfactor_L1_2)",ME21CSConly=False):
    ...
    for neta in range(len(netas)-1):
       ...
       for npar in allnpar:
		...
		ddY_cut = getcuts(filedirs_v6[0], treename, cuts_p[0], xaxis, pt, npar,netas[neta], netas[neta+1], fraction1, "(4000,0,40.0)", xtitle,"",outputdir2+"GEMCSC_ctau0_positionOnly_getcuts_recopt_20170131_ptcut%d_st2eta%dto%d_npar%d%s"%(pt, int(netas[neta]*10), int(netas[neta+1]*10), npar,extrapic))
		dphi_dir_cut = getcuts(filedirs_v6[0], treename, cuts_d[0], yaxis, pt, npar,netas[neta], netas[neta+1], fraction1, "(3000,0,1.0)", ytitle,"",outputdir2+"GEMCSC_ctau0_directionOnly_getcuts_recopt_20170131_ptcut%d_st2eta%dto%d_npar%d%s"%(pt, int(netas[neta]*10), int(netas[neta+1]*10), npar,extrapic))
		...
		Teffs_ddY = makeEffplot_v2(filedirs_v6, "pt", treename, cuts_p, ["fabs(%s)<%f"%(xaxis, ddY_cut), "fabs(%s)<%f"%(xaxis, ddY_cut)], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs,"Position-based algo" ,text_position,outputdir2+"GEMCSC_ctau0andctau1000_position_eff_20170131_pt%d_fraction%d_st2eta%dto%d_npar%d%s"%(pt, fraction1, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
		Teffs_dphi_dir = makeEffplot_v2(filedirs_v6, "pt", treename, cuts_d, ["fabs(%s)<%f"%(yaxis, dphi_dir_cut), "fabs(%s)<%f"%(yaxis, dphi_dir_cut)], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs,"Stub alignment algo",text,outputdir2+"GEMCSC_ctau0andctau1000_direction_eff_20170131_pt%d_fraction%d_st2eta%dto%d_npar%d%s"%(pt, fraction1, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
		astart = .6
		bstart = .0# not used 
		(maxa, maxb, alpha, x0, y0) = loopEllipse(filedirs_v6, treename, fraction, astart, bstart, xaxis, yaxis,x_bins, y_bins,xtitle, ytitle,st_title, netas[neta], netas[neta+1], dens_L1,text,outputdir1+"GEMCSC_ctau0andctau1000_hyrid_profile_20170131_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d%s"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
		...
		Teffs = makeEffplot_v2(filedirs_v6, "pt", treename, cuts, [ellipse, ellipse], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs,"Hybrid algo",text,outputdir2+"GEMCSC_ctau0andctau1000_hybrid_eff_20170131_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d%s"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
		...
		"""
		#hybridcut = "hybrid1"
		#Teffs = makeEffplot_v2(filedirs_v6, "pt", treename, cuts, [hybridcut, hybridcut], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs,"Hybrid algo",text,outputdir2+"GEMCSC_ctau0andctau1000_hybrid_eff_20170131_pt%d_ptbg%d_fraction%d_fractiond%d_st2eta%dto%d_npar%d_dirinterstation%s"%(pt, pt1, fraction, fraction_d, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
