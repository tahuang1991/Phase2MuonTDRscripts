...
def drawEllipse(hist, hist2, nEvent_list, ddYDPhiCuts, a, b, alpha, x0, y0, eff1, nrate, xtitle, ytitle,st_title, text, picname):
    ...
    xline1.Draw("same")
    xline2.Draw("same")
    print "Signal Acceptance: P ",acep_p, " D ",acep_d, " H ",eff1," this category ",nEvent_list[1]," Rejection: P ",rej_p," D ",rej_d," H ",rej_h
    tex2 = TLatex(0.15, 0.7, "Rejection %.3f(P), %.3f(D), %.3f(H)"%(rej_p, rej_d, rej_h))
    tex2.SetTextSize(0.05)
    tex2.SetNDC()
    #tex5 = TLatex(0.15, 0.9, "ellipse center(%.1f, %.3f)"%(x0, y0))
    tex5 = TLatex(0.15, 0.9, "Background entries %d"%(nEvent_list[1]))
...
def getEvents(ch, cut):
    hist = TH2F("hist","hist", 1,-10,10, 1, -1,1)
...
def loopEllipse(chain, chain1, nEvent_list, ddYDPhiCuts, fraction, npar, astart, bstart, xaxis, yaxis,x_bins, y_bins,xtitle, ytitle,st_title, etamin, etamax, cuts,text,picname):
    ...
    nEvent_list.append(0)#for hybrid, nEvent_list [4]
...
if len(sys.argv)>=4:
    ...
    pt1 = 10
    fraction = 95
    outputdir1 = "Profile_Ellipse_PT_20190303_00_eta12to14_frac95_testv4/"
    outputdir2 = "Hybrid_Ellipse_PT_20190303_00_eta12to14fract95_testv4/"
...
def plotalleta(pt, pt1, netas, fraction1 = 90, fraction=95, fraction_d = 99, yaxis = "(csc_bending_angle12_xfactor_L1_2)",ME21CSConly=False):
    ...
    xaxislist = [xaxis, xaxis]
    yaxislist = [yaxis, yaxis]
    ...
    addFilesToChain(GEMCSCTrackCh0, filedirs_v6[1]) 
    addFilesToChain(GEMCSCME1Ch0, filedirs_v6[1]) 
    addFilesToChain(GEMCSCME2Ch0, filedirs_v6[1]) 
    ...
    Teff_out = ROOT.TFile("Teffout_pt%d_eff%d_v4.root"%(pt, fraction),"RECREATE")
    for neta in range(len(netas)-1):
       ...
       for npar in allnpar:
		...
		cutsbase =  ["meRing==%d && "%ring+"hasSt1St2St3 && fabs(eta_st2_L1)>%f && fabs(eta_st2_L1)<%f && npar_L1==%d"%(netas[neta],netas[neta+1], npar)+" && (fabs(genGdMu_dxy)<5)", "meRing==%d && "%ring+"hasSt1St2St3 && fabs(eta_st2_L1)>%f && fabs(eta_st2_L1)<%f && fabs(genGdMu_dxy)>10 && fabs(genGdMu_dxy)<50 && fabs(genGdMu_dR)<0.1 && npar_L1==%d"%(netas[neta],netas[neta+1], npar)]
		...
		astart = .1
		bstart = .0# not used 
		cut_list = [cuts[0]+checkx+checky, cuts[1]+checkx+checky]
		(maxa, maxb, alpha, x0, y0) = loopEllipse(GEMCSCTrackCh0, GEMCSCTrackCh1, nEvent_list, ddYDPhiCuts, fraction, npar, astart, bstart, xaxislist, yaxislist,x_bins, y_bins,xtitle, ytitle,st_title, netas[neta], netas[neta+1], cut_list, text,outputdir1+"GEMCSC_ctau0andctau1000_hyrid_profile_20170131_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d%s"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
		...
		nEvent_list.append(getEvents(L1MuTriggerRate, ratecut))
		nEvent_list.append(getEvents(L1MuTriggerRate, ratecut + "&& abs(%s)<=%f"%(rate_x, ddY_cut)))
		nEvent_list.append(getEvents(L1MuTriggerRate, ratecut + "&& abs(%s)<=%f"%(rate_y, dphi_dir_cut)))
		nEvent_list.append(getEvents(L1MuTriggerRate, ratecut + "&& "+ellipsecut_rate))
		ratehist_p = TH1F("ratehist_p_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt),"rate hist, position",len(myetabin)-1, myetabin)	
		ratehist_d = TH1F("ratehist_d_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt),"rate hist, direction",len(myetabin)-1, myetabin)	
		ratehist_h = TH1F("ratehist_h_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt),"rate hist, hybrid",len(myetabin)-1, myetabin)	
		L1MuTriggerRate.Draw("abs(L1Mu_eta)>>"+"ratehist_p_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt), ratecut + "&& abs(%s)<=%f"%(rate_x, ddY_cut))
		L1MuTriggerRate.Draw("abs(L1Mu_eta)>>"+"ratehist_d_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt), ratecut + "&& abs(%s)<=%f"%(rate_y, dphi_dir_cut))
		L1MuTriggerRate.Draw("abs(L1Mu_eta)>>"+"ratehist_h_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt), ratecut + "&& "+ellipsecut_rate)
    		ratehist_p.Write()
    		ratehist_d.Write()
    		ratehist_h.Write()
		print "fianl nEvent_list ",nEvent_list
		...
		Teffallnpars_d.append(Teffs_dphi_dir)
	        Teffs[0].SetName("hybrideta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"promptmuonpt%dptbg%d"%(pt,pt1)+"fraction%d"%fraction)
	        Teffs_ddY[0].SetName("poistioneta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"promptmuonpt%dptbg%d"%(pt,pt1)+"fraction%d"%fraction)
	        Teffs_dphi_dir[0].SetName("directioneta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"promptmuonpt%dptbg%d"%(pt,pt1)+"fraction%d"%fraction)
	        Teffs[1].SetName("hybrideta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"displacedmuonpt%dptbg%d"%(pt,pt1)+"fraction%d"%fraction)
	        Teffs_ddY[1].SetName("poistioneta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"displacedmuonpt%dptbg%d"%(pt,pt1)+"fraction%d"%fraction)
	        Teffs_dphi_dir[1].SetName("directioneta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"displacedmuonpt%dptbg%d"%(pt,pt1)+"fraction%d"%fraction)
		Teffs[0].Write()
		Teffs_ddY[0].Write()
...
#plotalleta(pt1,pt2, [1.2,1.4],90, fraction, 99, "(dphi_dir_st1_st2_L1)",False)
#plotalleta(pt1,pt2, [1.6, 1.8, 2.0, 2.2],90, 95, 99, "(dphi_dir_st1_st2_L1)",False)
#plotalleta(pt1,pt2, [1.6, 1.8],90, 95, 99, "(dphi_dir_st1_st2_L1_csc)", True)
#plotalleta(pt1,pt2, [1.6, 1.8],90, fraction, 99, "(dphi_dir_st1_st2_L1_csc)", True)
