...
def printcuts(filedir, treename0, den, deltay12, deltay23, pt, npar,etamin, etamax, fractionToKeep, xtitle, ytitle, txt, picname, LUT):
		...
		upperlim = getUpperlimit(hist, fractionToKeep)
		LUT.write("eta%dto%dnpar%d%spt%dfraction%d:%f"%(int(etamin*10), int(etamax*10), npar,num,pt,fractionToKeep,upperlim)+"\n")
		c1 = ROOT.TCanvas()
		c1.SetGridx()
		c1.SetGridy()
		c1.SetTickx()
...
def printallcuts(filedir, treename0, den, deltay12, deltay23, netas, pts, fractionToKeep, xtitle, ytitle, txt,picname, LUT):
    evenodds = ["odd,even","odd,odd","even,even","even,odd","all pairs"]
    for pt in pts:
		...
		for npar in allnpar:
			...
			den1 = den+"&& fabs(eta_st2_sh)>%f && fabs(eta_st2_sh)<%f && meRing==%d"%(netas[neta], netas[neta+1],ring)
		        printcuts(filedir, treename0, den1, deltay12,deltay23, pt, npar,etamin,etamax, fractionToKeep, xtitle, ytitle, txt1, picname1, LUT)
...
def makeEffplot_v2(filedirs,todraw, treename0, den, deltay12, deltay23, pt, allnpar, fractionToKeep, etamin, etamax, xtitle,ytitle,leg, txt,picname):
	...
	for n in range(len(filedirs)):
		...
		filedir = filedirs[n]
    		chain = ROOT.TChain(treename0)
#		treename2 = "HLTBendingAngle/trk_eff_csc_ME21" 
   		chain2 = ROOT.TChain("GEMCSCAnalyzer/trk_eff_CSC_ME11")
   		chain3 = ROOT.TChain("GEMCSCAnalyzer/trk_eff_CSC_ME2%d"%ring)#take care of eta in station2
   		chain4 = ROOT.TChain("GEMCSCAnalyzer/trk_eff_CSC_ME3%d"%ring)
    		if os.path.isdir(filedir):
    			ls = os.listdir(filedir)
    	 		for x in ls:
		...
		for xpar in range(len(allnpar)):
		     ...
		     if n==0:
			hist = ROOT.TH1F("hist","hist",4000,0.0,40)
        		chain.Draw("TMath::Abs(%s)>>hist"%(num),den[n]+"&& npar_lct==%d && pt>%f && pt<%f && TMath::Abs(%s)<40"%(npar,pt-1, pt+1, num))
       			print "cuts for upperlim ",den[n]+"&& npar_lct==%d && pt>%f && pt<%f &&  TMath::Abs(%s)<40"%(npar,pt-1, pt+1, num)+deltay
       			#hist.Print("ALL")
			upperlim = getUpperlimit(hist, fractionToKeep)
			...
			print "npar%d%spt%dfraction%d:%f"%(npar,num,pt,fractionToKeep,upperlim)
		     ...
		     hdens[n].Add(gethist1D(chain, den[n]+"&& npar_lct==%d &&  fabs(%s)<40"%(npar, num)+deltay,todraw))
	...
	#tex = ROOT.TLatex(0.15,0.87,"%s"%txt)
	tex = ROOT.TLatex(0.25,0.6,"#splitline{%s}{p_{T}^{L1}>%d GeV}"%(txt, pt))
...
LUT = open("Position_LUT_1010.log","w+")
printallcuts(filedir16, treename0,"hasSt1St2St3", "deltay12_fit", "deltay23_fit", netas, [2,3,4,5,7,10,15,20,30,40], 90, "|#Delta#Delta Y|", "Entries", "digi level ","Position_PT_Cut_0909/GEMCSC_ctau0_PositionOnly_20160909", LUT)
if len(sys.argv)>=3:
...
def plotalleta(pt, netas, fraction=90):
	...
	me11 = evenodds[npar].split(',')[0]
	me21 = evenodds[npar].split(',')[1]
	eta_st2 = "fabs(trk_eff_CSC_ME2%d.eta_lct_%s)"%(ring, me21)
	eta_cut = "&& %s>%f && %s<%f"%(eta_st2, netas[neta], eta_st2, netas[neta+1])
	chambers = "ME1%d %s,ME2%d %s,ME3%d %s"%(ring, me11, ring, me21, ring, me21)
	dens_L1 =  ["hasSt1St2St3 && meRing==%d"%(ring)+eta_cut,"hasSt1St2St3 && fabs(genGdMu_dxy)>10 && fabs(genGdMu_dxy)<50 && fabs(genGdMu_dR)<0.1 && meRing==%d"%(ring)+eta_cut]
