...
//=========  (Tue Jul 25 12:40:39 2017) by ROOT version6.08/06
   ...
   hs_stack_31->GetYaxis()->SetLabelSize(0.05);
   ...
   hrate_p_fractionrate96__391->SetLineWidth(2);
   ...
   hrate_h_fractionrate96__392->SetLineWidth(2);
   ...
   TLegend *leg = new TLegend(0.24,0.3,0.85,0.58,NULL,"brNDC");
   leg->SetBorderSize(1);
   //leg->SetTextSize(.042);
   leg->SetLineColor(1);
   leg->SetLineStyle(1);
   ...
   entry=leg->AddEntry("hrate_p_fractionrate96","Phase-1(CSC): positions only","pl");
   ...
   entry->SetMarkerSize(1);
   entry->SetTextFont(42);
   entry=leg->AddEntry("hrate_h_fractionrate96","#splitline{Phase-2(CSC+GE11+GE21):}{ positions and directions}","pl");
   entry->SetLineColor(1);
   entry->SetLineStyle(1);
   entry->SetLineWidth(2);
   entry->SetMarkerColor(1);
   ...
   pt->Draw();
      tex = new TLatex(0.2,0.2,"1.65<|#eta|<2.10, L1 track based veto not applied");
tex->SetNDC();
