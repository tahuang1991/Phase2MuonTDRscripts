...
import numpy as np
from math import *
from GEMCSCdPhiDict_wholeChamber_20170221 import *
...
def getDPhiFromLUT(st, pt, fraction, evenodd):
    ...
    ptstring = "Pt%d"%pt
    return dphi_lct_pad[ststring][fracstring][ptstring][evenodd]
...
def getBackgroundRejectionEllipse(a_axis, b_axis, alpha, x0, y0, signalHist, backgroundHist):
    ...
    backgroundRejectionFactor = 1 - entriesInEllipseBackground/backgroundEntriesTotal
...
def drawEllipse(hist, hist2, nEvent_list, anglecut_list, a, b, alpha, x0, y0, eff1, eff2, xtitle, ytitle,st_title, text, picname):
    ...
    acep_x = float(nEvent_list[5])/float(nEvent_list[4])
    acep_y = float(nEvent_list[6])/float(nEvent_list[4])
    tex1 = ROOT.TLatex(0.15,.3,"%s"%(text))
    tex1.SetNDC()
    tex10 = TLatex(0.2,.9,"Singal entries %d"%(nEvent_list[4]))
    tex10.SetNDC()
    tex3 = TLatex(0.15, 0.7, "Acceptance %.3f(ST1) %.3f(ST2) %.3f(H)"%(acep_x, acep_y, eff1))
    tex3.SetTextSize(0.05)
    tex3.SetNDC()
    tex4 = TLatex(0.15, 0.8, "#splitline{HCut: a=%.3f,b=%.3f,angle=%.3f,center(%.3f,%.3f)}{St1-Cut: %.2f, St2-Cut: %.3f}"%(a, b, alpha, x0, y0, anglecut_list[0],anglecut_list[1]))
    ...
    xline1.Draw("same")
    xline2.Draw("same")
    print "Singal Acceptance: X ",acep_x, " Y ",acep_y, " H ",eff1, " Rate nEvent ",nEvent_list[0]," this category ",nEvent_list[1]," Rejection: X ",rej_x," Y ",rej_y," H ",eff2
    #tex2 = TLatex(0.15, 0.8, "a=%.1f, b=%.3f, alpha=%.4f, Rejection %.2f"%(a,b,alpha, eff2))
    tex2 = TLatex(0.15, 0.7, "Rejection %.3f(ST1), %.3f(ST2), %.3f(H)"%(rej_x, rej_y, eff2))
...
def loopEllipse(filedir, treename,b0, nEvent_list, anglecut_list, fraction, astart, bstart, xaxis, yaxis,x_bins, y_bins,xtitle, ytitle,st_title, etamin, etamax, cuts,text,picname):
    ...
    xcut = anglecut_list[0]
    ycut = anglecut_list[1]
    binx1 = b0.GetXaxis().FindBin(-xcut)
    binx2 = b0.GetXaxis().FindBin(xcut)
    biny1 = b0.GetYaxis().FindBin(-ycut)
    biny2 = b0.GetYaxis().FindBin(ycut)
    nEvent_list.append(b0.Integral(binx1, binx2, 0, yBins+1))#rate, overflow and underflow are included
    nEvent_list.append(b0.Integral(0, xBins+1, biny1, biny2))
    a_range = frange(astart, xmaxBin/2.0, xbinwidth)
    ...
    todrawb_hist = "(%s*(abs(%s)<0.1)+0.02*(abs(%s)>.1))"%(yaxis, yaxis, yaxis)+":"+"(%s*(abs(%s)<0.1)+0.02*(abs(%s)>.1))"%(xaxis, xaxis, xaxis)
    ...
    chain.Draw(todrawb01,cuts[0],"colz")#background, for plotting
    #b0.Add(b01)
    ...
    chain1.Draw(todrawb11,cuts[1],"colz")#signal, for plotting
    #b1.Add(b11)
    print "signal todraw ",todrawb1, " cuts ", cuts[1]," b1.Getentries ",b1.GetEntries()," b0.GetEntries ",b0.GetEntries()
    nEvent_list.append(b1.GetEntries())
    nEvent_list.append(b1.Integral(binx1, binx2, 0, yBins+1))#overflow underflow included
    nEvent_list.append(b1.Integral(0, xBins+1, biny1, biny2))
    print "nEvent_list ",nEvent_list," anglecuts ", anglecut_list
    print "binx1 ",binx1," binx2 ",binx2, " yBIns ",yBins, " xBins ",xBins
    ...
    drawEllipse(b11, b0,nEvent_list, anglecut_list, max_a, max_b, 0, centerx, centery, signalAcceptanceFactor, backgroundRejectionFactor, xtitle, ytitle,st_title, text, picname+"_%d_initial"%(lena*lenb))
    totalnalpha = 7
    ...
    #for xratio in [0.0]:
    nalpha0 = 1
    for nalpha in range(0,nalpha0):
       alpha = 3.1415926/(2*nalpha0)*nalpha
	...
	while (step> ybinwidth):
	    ...
	    n = n+1
			...
			max_centerx = centerx
			max_centery = centery
    	    		drawEllipse(b11, b0, nEvent_list, anglecut_list, a_axis, b_axis, max_alpha, max_centerx, max_centery, signalAcceptanceFactor, backgroundRejectionFactor, xtitle, ytitle,st_title, text, picname+"_nalpha%d_m%d_n%d"%(nalpha, m,n))
...
def makeEffplot_v2(filedirs,todraw, treename0, den, num, etamin, etamax, xtitle,ytitle,leg, legheader, txt,picname):
	...
	print "makeEffplot_v2 filedirs ",filedirs
...
filedir16 = "/eos/uscms/store/user/tahuang/SLHC23_patch1_2023Muon_gen_sim_Pt2_50_1M/GEMCSC_Ana_ctau0_Pt2_50_PU140_20160221/170222_055841/0000/combined/out_ana_prompt.root"
...
if len(sys.argv)>=5:
	...
	pt = 10
	pt1 = 7
	outputdir1 = "Profile_Ellipse_GEMCSC_20160208_00_test/"
	outputdir2 = "Hybrid_Ellipse_GEMCSC_20170208_00_test/"

#pt1 = 10
#pt2 = 7
LUT = open("GEMCSCBendingHybridLUT_Pt%d_BGPt%d_20170208.log"%(pt, pt1),"w+")
#LUT_st1 = open("GEMCSCBendingST1LUT_Pt%d_20170208.log"%(pt),"w+")
#LUT_st2 = open("GEMCSCBendingST2LUT_Pt%d_20170208.log"%(pt),"w+")
#for npt in range(len(Pts)):	
def plotalleta(pt, pt1, netas, fraction = 98):
    Teffs_0 = []
    Teffs_0_ME11 = []
    Teffs_0_ME21 = []
    extrapic=""
    ...
    L1MuTriggerRate = ratefile.Get("L1MuTriggerRate")
    ...
    for neta in range(len(netas)-1):
       ...
       for npar in allnpar:
		...
		legs = ["prompt muon","prompt muon"]
		etacuts ="fabs(trk_eff_CSC_ME2%d.eta_lct_%s)>%f && fabs(trk_eff_CSC_ME2%d.eta_lct_%s)<%f"%(ring,me21,netas[neta],ring,me21,netas[neta+1])
		...
		#yaxis = "(csc_bending_angle12_xfactor_L1_2)"#deltaphi, only in high eta region, CSC only
    		...
    		checkvalue = "has_tfTrack && fabs(%s)<.1 && fabs(%s)<.1 && meRing==%d && trackpt>=%f"%(xaxis, yaxis,ring, pt)
		...
		checkx = "&& fabs(%s)<.1"%(xaxis)
		checky = "&& fabs(%s)<.1"%(yaxis)
		cutsbase =  [etacuts, etacuts]
		...
		ratecuts = "maxPromptPt && abs(L1Mu_eta)>%f && abs(L1Mu_eta)<%f && npar_bending==%d && L1Mu_pt>=%f"%(etamin, etamax, npar, pt)
    		#h_rate_string = "h_displacedMu_dPhi_ddY_%s_eta%dto%d_npar%d"%(GE21string, int(etamin*10), int(etamax*10),npar)
		h_rate = TH2F("h_rate","rate sample, npar=%d"%npar,400, -.04, 0.04, 300, -0.03,0.03)
		L1MuTriggerRate.Draw(ratetodraw+">>h_rate",ratecuts)
    		print "rate histogram , entries ",h_rate.GetEntries()

		text = "#splitline{%s}{%.1f<|#eta|<%.1f, p_T^{L1}>%d GeV}"%(chambers, netas[neta],netas[neta+1], pt)
		#xcut = getcuts(filedirs_v6[1], treename, etacuts+checkx, xaxis, pt, npar, netas[neta], netas[neta+1], fraction, "(5000,0,.10)", xtitle,"",outputdir2+"GEMCSC_St1_ctau0_getcuts_recopt_20170208_ptcut%d_st2eta%dto%d_npar%d"%(pt, int(netas[neta]*10), int(netas[neta+1]*10), npar))
		#ycut = getcuts(filedirs_v6[1], treename, etacuts+checky, yaxis, pt, npar, netas[neta], netas[neta+1], fraction, "(5000,0,.10)", ytitle,"",outputdir2+"GEMCSC_St2_ctau0_getcuts_recopt_20170208_ptcut%d_st2eta%dto%d_npar%d"%(pt, int(netas[neta]*10), int(netas[neta+1]*10), npar))
		#xcut = getDPhicut(1, pt, me11=="odd")
		#ycut = getDPhicut(2, pt, me21=="odd")
		xcut = getDPhiFromLUT(1, pt, fraction, me11)
		ycut = getDPhiFromLUT(2, pt, fraction, me21)
		...
		if npar==3:
			extra_comma = ""
		#LUT_st1.write("%f"%xcut+extra_comma)
		#LUT_st2.write("%f"%ycut+extra_comma)
		Teffs_cutx = makeEffplot_v2(filedirs_v6, "pt", treename, [etacuts+checkx, etacuts+checkx], ["fabs(%s)<%f"%(xaxis, xcut), "fabs(%s)<%f"%(xaxis, xcut)], netas[neta], netas[neta+1],"true muon p_{T} GeV","bending angle cut efficiency",legs,"GE11-ME11" ,text,outputdir2+"GEMCSC_St1_eff_20170208_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
		Teffs_cuty = makeEffplot_v2(filedirs_v6, "pt", treename, [etacuts+checky, etacuts+checky], ["fabs(%s)<%f"%(yaxis, ycut), "fabs(%s)<%f"%(yaxis, ycut)], netas[neta], netas[neta+1],"true muon p_{T} GeV","bending angle cut efficiency",legs,"GE21-ME21" ,text,outputdir2+"GEMCSC_St2_eff_20170208_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
    		#h_rate_string = "h_dphi_ME11_ME21_eta%dto%d_npar%d"%(int(etamin*10), int(etamax*10),npar)
    		h_rate_string = "h_dphi_ME11_ME21_L1pt%d_eta16to22_npar%d"%(int(pt),npar)
		...
		astart = .0001
		bstart = .0# not used 
		(maxa, maxb, alpha, x0, y0) = loopEllipse(filedirs_v6, treename, h_rate, nEvent_list, anglecut_list, fraction, astart, bstart, xaxis, yaxis,x_bins, y_bins,xtitle, ytitle,st_title, netas[neta], netas[neta+1], dens_L1,text,outputdir1+"GEMCSC_St1St2_hyrid_profile_20170208_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
		...
		Teffs = makeEffplot_v2(filedirs_v6, "pt", treename, cuts, [ellipse, ellipse], netas[neta], netas[neta+1],"true muon p_{T} GeV","bending angle cut efficiency",legs,"GE11-ME11 and GE21-ME21",text,outputdir2+"GEMCSC_St1St2_hybrid_eff_20170208_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
		#Teffs = makeEffplot_v2(filedirs_v6, "pt", treename, cuts, [ellipse+"&& fabs(%s)<%f"%(xaxis, ddY_cut)+" && fabs(%s)<%f"%(yaxis, dphi_dir_cut), ellipse+" && fabs(%s)<%f"%(xaxis, ddY_cut)+"&& fabs(%s)<%f"%(yaxis, dphi_dir_cut)], netas[neta], netas[neta+1],"true muon p_{T} GeV","Trigger efficiency",legs,"Hybrid algo",text,outputdir2+"GEMCSC_ctau0andctau1000_hybrid_eff_20170120_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d%s_addpositionAnddirection"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar, extrapic))
       ...
       Teff0.SetName("hybrideta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"BGpt%dptbg%d"%(pt,pt1))
       Teff1.SetName("hybrideta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"Singalpt%dptbg%d"%(pt,pt1))
       Teff0_st1.SetName("ME11eta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"BGpt%dptbg%d"%(pt,pt1))
       Teff1_st1.SetName("ME11eta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"Singalpt%dptbg%d"%(pt,pt1))
       Teff0_st2.SetName("ME21eta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"BGpt%dptbg%d"%(pt,pt1))
       Teff1_st2.SetName("ME21eta%dto%d"%(int(netas[neta]*10),int(netas[neta+1]*10))+"Singalpt%dptbg%d"%(pt,pt1))
       text_h = "#splitline{GE11-ME11+GE21-ME21}{%.1f<|#eta|<%.1f, p_{T}^{L1}>%d GeV%s}"%(netas[neta],netas[neta+1], pt, extratxt)
       text_ME11 = "#splitline{GE11-ME11}{%.1f<|#eta|<%.1f, p_{T}^{L1}>%d GeV%s}"%(netas[neta],netas[neta+1], pt, extratxt)
       text_ME21 = "#splitline{GE21-ME21}{%.1f<|#eta|<%.1f, p_{T}^{L1}>%d GeV%s}"%(netas[neta],netas[neta+1], pt, extratxt)
       ...
       makeplots([Teff0_st1, Teff0_st2, Teff0], legs3, text3,outputdir2+"GEMCSC_GE11ME11_GE21ME21_combined3_eff_20170120_pt%d_ptbg%d_fraction%d_St2eta%dto%d_allnpar_dirinterstation%s"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), extrapic)) 
...
plotalleta(pt, pt1, [1.6,2.2], fraction)
