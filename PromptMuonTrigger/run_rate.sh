...
set pythonfile = GEMCSC_bendingangle_rate.py
set runfile = makeplotrate.sh
set condoroutput = condor_GEMCSC_20190304_rate_floatpt_eta164to215
set condorlog = condorout
#set Pts = (15 20)
set Pts = (5 6 7 8 10 12 14 16 20)
set Pts_BG = (5)
set fractions = (90 91 92 93 94 95 96 97 98 99)
#set Pts = (5 6 7 8 9 10 12 15 20 30 40)
#set Pts_BG = (5 5 5 6 7 7 9 10 12 20 20)
...
foreach pt1 ($Pts)
	...
	cp $workdir/submit_condor_jobs_rate_pt10.cmd submit_condor_jobs_rate_pt$pt1.cmd
	cp $workdir/$runfile .
	cp $workdir/$pythonfile .
	set submitdir = $PWD
	if (! -d $condorlog) then
	    mkdir $condorlog 
	endif
	echo counter $counter, Pt1 $pt1, fracction $frac, submitdir , $submitdir
#cp submit_condor_jobs_module_pt$pt1.cmd submit_condor_jobs_copy.cmd
# changes the the flag FNAME by one of the input 
# file names in the list
	sed -i "s/PT1/$pt1/g" submit_condor_jobs_rate_pt$pt1.cmd
	sed -i "s/fraction/$frac/g" submit_condor_jobs_rate_pt$pt1.cmd
...
#sed -i "s/arguments123/$pt1 $pt2 $submitdir/g" submit_condor_jobs_copy.cmd
	cat submit_condor_jobs_rate_pt$pt1.cmd
	condor_submit submit_condor_jobs_rate_pt$pt1.cmd
    end
#shift Pts_BG
#done
end
