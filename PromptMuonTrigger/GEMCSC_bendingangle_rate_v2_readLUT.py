...
def makeEllipse(chain, chain1, nEvent_list, anglecut_list, fraction, astart, bstart, xaxis, yaxis,x_bins, y_bins,xtitle, ytitle,st_title, etamin, etamax, cuts,text,picname):
	...
	#print "signal ellipse ",cuts[1]+"&&"+ellipse_s," nsignal ",nsignal
	nrate = getEvents(chain, cut_b)
        print "ellipse cut %f %f %f %f %f "%(maxa, maxb, alpha, x0, y0)," signal ",nsignal, " nrate ",nrate
	return float(nsignal)/float(totsignal),nrate
...
if len(sys.argv)>=5:
	...
	pt = 7
	pt1 = 0
	outputdir1 = "Profile_Ellipse_GEMCSC_20170312_00_test/"
	outputdir2 = "Hybrid_Ellipse_GEMCSC_20170312_00_test/"
    ...
    for neta in range(len(netas)-1):
       ...
       for npar in allnpar:
		...
		#yaxis = "(csc_bending_angle12_xfactor_L1_2)"#deltaphi, only in high eta region, CSC only
    		...
    		#checkvalue = "&& %s<1 && %s<1"%(xaxis, yaxis)
		...
		cutsbase =  [etacuts + "&& has_tfTrack>0 && has_gmtCand>0 && nstubs>=2 && meRing==%d "%(ring)+checkx_sh+checky_sh, etacuts + "&& has_tfTrack>0 && has_gmtCand>0 &&nstubs>=2 && meRing==%d"%(ring)+checkx_sh+checky_sh]
		...
		#xcut_lut = getDPhiFromLUT(1, pt, fraction, me11)
		basecut = etacuts + "&& has_tfTrack>0 && has_gmtCand>0 && nstubs>=2 && meRing==%d "%(ring)
    		xcut = getCut(GEMCSCTrackCh0, xaxis, basecut+"&& fabs(%s)<1"%(xaxis)+" && L1Mu_pt>=%f && pt>=%f"%(pt, pt), 0.0, 0.05, 0.00001, fraction)
		#ycut_lut = getDPhiFromLUT(2, pt, fraction, me21)
    		ycut = getCut(GEMCSCTrackCh0, yaxis, basecut+"&& fabs(%s)<1"%(yaxis)+" && L1Mu_pt>=%f && pt>=%f"%(pt, pt), 0.0, 0.05, 0.00001, fraction)
		...
		#Teffs_me11 = makeEffplot_v2([GEMCSCTrackCh0, GEMCSCTrackCh1], "pt", [cutsbase[0]+checkx_sh+checky_sh, cutsbase[1]+checkx_sh+checky_sh], ["hasME1", "hasME1"], netas[neta], netas[neta+1],"true muon p_{T} GeV","bending angle cut efficiency",legs,"has Stub in ME11" ,text,outputdir2+"GEMCSC_TrackhasME11_eff_20170208_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
		#Teffs_me21 = makeEffplot_v2([GEMCSCTrackCh0, GEMCSCTrackCh1], "pt", [cutsbase[0]+checkx_sh+checky_sh, cutsbase[1]+checkx_sh+checky_sh], ["hasME2", "hasME2"], netas[neta], netas[neta+1],"true muon p_{T} GeV","bending angle cut efficiency",legs,"has Stub in ME21" ,text,outputdir2+"GEMCSC_TrackhasME21_eff_20170208_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
		#should be consistent with 98%
		Teffs_cutx = makeEffplot_v2([GEMCSCTrackCh0], "pt", [cutsbase[0]], ["abs(%s)<%f && L1Mu_pt>=%f"%(xaxis, xcut, pt)], netas[neta], netas[neta+1],"true muon p_{T} GeV","bending angle cut efficiency",legs,"GE11-ME11" ,text,outputdir2+"GEMCSC_St1_GE11cut_eff_20170208_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
		Teffs_cuty = makeEffplot_v2([GEMCSCTrackCh0], "pt", [cutsbase[0]], ["abs(%s)<%f && L1Mu_pt>=%f"%(yaxis, ycut, pt)], netas[neta], netas[neta+1],"true muon p_{T} GeV","bending angle cut efficiency",legs,"GE21-ME21" ,text,outputdir2+"GEMCSC_St2_GE21cut_eff_20170208_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
		#should be less than 98%
		nEvent_list = []
		nEvent_list.append(rateEvents)
    	        anglecut_list = [xcut, ycut]

		cutslist = [ratecut, cutsbase[1]+"&& pt>=%f"%(pt)+"&& L1Mu_pt>=%f"%(pt)]
		...
		ratehist_st1 = getRateVsEta(L1MuTriggerRate, "ratehist_GE11_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt), myetabin, ratecut + "&& abs(%s)<=%f"%(rate_x, xcut))
		ratehist_st2 = getRateVsEta(L1MuTriggerRate, "ratehist_GE21_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt), myetabin, ratecut + "&& abs(%s)<=%f"%(rate_y, ycut))
		...
		ratehist_h = getRateVsEta(L1MuTriggerRate, "ratehist_hybrid_eta%dto%d_npar%d_frac%d_pt%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar, fraction, pt), myetabin, ratecut_ellipse)
		print "rate ge11 ",ratehist_st1.GetEntries()," ge21 ",ratehist_st2.GetEntries()," hybrid ",ratehist_h.GetEntries()
		ratehist_st1.Write()
		ratehist_st2.Write()
		ratehist_h.Write()
    		Teffs_cutx[0].SetName("GE11eta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"pt%d"%(pt)+"fraction%d"%fraction)
    		Teffs_cuty[0].SetName("GE21eta%dto%dnpar%d"%(int(netas[neta]*10),int(netas[neta+1]*10), npar)+"pt%d"%(pt)+"fraction%d"%fraction)
...
def plotalleta_FromLUT( ,pt, pt1, netas, fraction = 98):
    Teffs_0 = []
    Teffs_0_ME11 = []
    Teffs_0_ME21 = []
    extrapic=""
...
plotalleta(pt, pt1, [1.64,2.15], fraction)
