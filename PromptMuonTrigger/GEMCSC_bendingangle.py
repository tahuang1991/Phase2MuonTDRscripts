...
def drawEllipse(hist, hist2, a, b, alpha, x0, y0, eff1, eff2, xtitle, ytitle,st_title, text, picname):
    ...
    angle = "%.3f"%alpha
    print "in Drawellipse a ",a," b ",b," alpha ",alpha," angle ", alpha*180.0/pi, " x0 ",x0, " y0 ",y0," text ",text
    tex1 = ROOT.TLatex(0.15,.7,"%s"%(text))
    #tex1 = TLatex(0.2,.7,"%s"%(st_title))
    tex1.SetNDC()
    tex3 = TLatex(0.13, 0.8, "a=%.3f, b=%.3f, alpha=%.3f, Acceptance %.2f"%(a,b,alpha, eff1))
    tex3.SetTextSize(0.05)
    tex3.SetNDC()
    tex4 = TLatex(0.15, 0.9, "ellipse center(%.3f, %.3f)"%(x0, y0))
    ...
    btest2.SetTitle(hist2.GetTitle())
    btest2.Draw()
    hist2.Draw("samecolz")
    hist2.SetTitle(st_title[0])
    el2.Draw("same")
    tex2 = TLatex(0.13, 0.8, "a=%.3f, b=%.3f, alpha=%.3f, Rejection %.2f"%(a,b,alpha, eff2))
    tex2.SetTextSize(0.05)
    tex2.SetNDC()
    tex5 = TLatex(0.15, 0.9, "ellipse center(%.3f, %.3f)"%(x0, y0))
...
def loopEllipse(filedir, treename,fraction, astart, bstart, xaxis, yaxis,x_bins, y_bins,xtitle, ytitle,st_title, etamin, etamax, cuts,text,picname):
    ...
    drawEllipse(b11, b01, max_a, max_b, 0, centerx, centery, signalAcceptanceFactor, backgroundRejectionFactor, xtitle, ytitle,st_title, text, picname+"_%d_initial"%(lena*lenb))
    totalnalpha = 7
	...
	while (step> ybinwidth):
	    ...
	    n = n+1
			...
			max_centerx = centerx
			max_centery = centery
    	    		drawEllipse(b11, b01, a_axis, b_axis, max_alpha, max_centerx, max_centery, signalAcceptanceFactor, backgroundRejectionFactor, xtitle, ytitle,st_title, text, picname+"_nalpha%d_m%d_n%d"%(nalpha, m,n))
...
treename = "GEMCSCAnalyzer/trk_eff_CSC_ME11"
...
#filedir16 = "/eos/uscms/store/user/tahuang/SLHC23_patch1_2023Muon_gen_sim_Pt2_50_1M/GEMCSCAna_ctau0_Pt2_50_0929_v2/160926_194628/0000/"
#filedir16 = "/eos/uscms/store/user/tahuang/SLHC23_patch1_2023Muon_gen_sim_Pt2_50_1M/GEMCSCAna_ctau0_Pt2_50_0929/160930_022125/0000/"
#filedir16 = "/eos/uscms/store/user/tahuang/SLHC23_patch1_2023Muon_gen_sim_Pt2_50_1M/GEMCSCAna_ctau0_Pt2_50_1016_direction/161017_052319/0000/"
filedir16 = "/eos/uscms/store/user/tahuang/SLHC23_patch1_2023Muon_gen_sim_Pt2_50_1M/GEMCSCAna_ctau0_Pt2_50_1018/161019_055433/0000/"
...
#filedir46 = "/eos/uscms/store/user/tahuang/DarkSUSY_MH-125_MGammaD-20000_ctau1000_14TeV_madgraph-pythia6-tauola/GEMCSCAna_DarkSUSY_ctau1000_0929_v2/160926_194711/0000/"
#filedir46 = "/eos/uscms/store/user/tahuang/DarkSUSY_MH-125_MGammaD-20000_ctau1000_14TeV_madgraph-pythia6-tauola/GEMCSCAna_DarkSUSY_ctau1000_0929/160930_021940/0000/"
#filedir46 = "/eos/uscms/store/user/tahuang/DarkSUSY_MH-125_MGammaD-20000_ctau1000_14TeV_madgraph-pythia6-tauola/GEMCSCAna_DarkSUSY_ctau1000_1016_direction/161017_035825/0000/"
filedir46 = "/eos/uscms/store/user/tahuang/DarkSUSY_MH-125_MGammaD-20000_ctau1000_14TeV_madgraph-pythia6-tauola/GEMCSCAna_DarkSUSY_ctau1000_1018/161019_055236/0000/"
...
if len(sys.argv)>=5:
	...
	outputdir1 = "Profile_Ellipse_GEMCSC_20161212_00_norotation/"
	outputdir2 = "Hybrid_Ellipse_GEMCSC_20161212_00_norotation/"

#pt1 = 10
#pt2 = 7
LUT = open("GEMCSCBendingHybridLUT_Pt%d_BGPt%d_20161212.log"%(pt1, pt2),"w+")
LUT_st1 = open("GEMCSCBendingST1LUT_Pt%d_20161212.log"%(pt1),"w+")
LUT_st2 = open("GEMCSCBendingST2LUT_Pt%d_20161212.log"%(pt1),"w+")
    ...
    for neta in range(len(netas)-1):
       ...
       for npar in allnpar:
		...
		#yaxis = "(csc_bending_angle12_xfactor_L1_2)"#deltaphi, only in high eta region, CSC only
    		...
    		checkvalue = "fabs(%s)<.1 && fabs(%s)<.1 && meRing==%d"%(xaxis, yaxis,ring)
		...
		x_bins = "(200,-0.05,0.05)"
		y_bins = "(200,-0.05,0.05)"
		...
		xcut = getcuts(filedirs_v6[1], treename, etacuts+checkx, xaxis, pt, npar, netas[neta], netas[neta+1], fraction, "(5000,0,.10)", xtitle,"",outputdir2+"GEMCSC_St1_ctau0_getcuts_recopt_20161212_ptcut%d_st2eta%dto%d_npar%d"%(pt, int(netas[neta]*10), int(netas[neta+1]*10), npar))
		ycut = getcuts(filedirs_v6[1], treename, etacuts+checky, yaxis, pt, npar, netas[neta], netas[neta+1], fraction, "(5000,0,.10)", ytitle,"",outputdir2+"GEMCSC_St2_ctau0_getcuts_recopt_20161212_ptcut%d_st2eta%dto%d_npar%d"%(pt, int(netas[neta]*10), int(netas[neta+1]*10), npar))
		...
		Teffs_cutx = makeEffplot_v2(filedirs_v6, "pt", treename, [etacuts+checkx, etacuts+checkx], ["fabs(%s)<%f"%(xaxis, xcut), "fabs(%s)<%f"%(xaxis, xcut)], netas[neta], netas[neta+1],"true muon p_{T} GeV","bending angle cut efficiency",legs,"GE11-ME11" ,text,outputdir2+"GEMCSC_St1_eff_20161212_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
		Teffs_cuty = makeEffplot_v2(filedirs_v6, "pt", treename, [etacuts+checky, etacuts+checky], ["fabs(%s)<%f"%(yaxis, ycut), "fabs(%s)<%f"%(yaxis, ycut)], netas[neta], netas[neta+1],"true muon p_{T} GeV","bending angle cut efficiency",legs,"GE21-ME21" ,text,outputdir2+"GEMCSC_St2_eff_20161212_pt%d_fraction%d_st2eta%dto%d_npar%d"%(pt, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
		astart = .0001
		bstart = .0# not used 
		(maxa, maxb, alpha, x0, y0) = loopEllipse(filedirs_v6, treename, fraction, astart, bstart, xaxis, yaxis,x_bins, y_bins,xtitle, ytitle,st_title, netas[neta], netas[neta+1], dens_L1,text,outputdir1+"GEMCSC_St1St2_hyrid_profile_20161212_pt%d_ptbg%d_fraction%d_st2eta%dto%d_npar%d"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar))
		...
		legs3 = ["GE11-ME11","GE21-ME21","GE11-ME11+GE21-ME21"]
       		makeplots([Teffs_cutx[0], Teffs_cuty[0], Teffs[0]], legs3, text,outputdir2+"GEMCSC_St1St2_combined3_eff_20161212_pt%d_ptbg%d_fraction%d_St2eta%dto%d_npar%d"%(pt, pt1, fraction, int(netas[neta]*10),int(netas[neta+1]*10), npar)) 
